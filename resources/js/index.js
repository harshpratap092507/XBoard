...
function createCard(item) {
  return `<div class="card d-block">
                  <img src="${
                    item.enclosure.link
                  }" class="card-img-top img-fluid carousel-img" alt="card image">
                  <div class="card-body">
                      ...
                      <p class="card-subtitle text-secondary d-inline">${new Date(
                        item.pubDate
...
async function loadContent() {
  ...
  for (let i = 0; i < magazines.length; i++) {
    ...
    for (let item = 0; item < items.length; item++) {
      ...
      let innerCarouselCard = createInnerCarousel(
        innerCarouselCardId,
        item == 0
      );
